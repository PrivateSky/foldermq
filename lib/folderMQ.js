...
function FolderMQ(folder, callback = () => {}){
	...
	var recipient;
	this.setIPCChannel = function(process){
		if(process && !process.send || (typeof process.send) != "function"){
			throw new Error("Recipient is not instance of process/child_process or it was not spawned with IPC channel!");
		}
		recipient = process;
	}

	function buildEnvelopeConfirmation(envelope){
	...
	this.registerAsIPCConsumer = function(callback){
        ...
        function receiveEnvelope(envelope){
			...
			if(callback){
                ...
                recipient.send(buildEnvelopeConfirmation(envelope));
	...
	function writeFile(filename, content, callback){
    	if(recipient){
    		var envelope = buildEnvelope(content);
